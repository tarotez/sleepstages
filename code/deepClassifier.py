...
class DeepClassifier():
    ...
    def generateModel(self):
        ...
        if params.extractorType.startswith('rawData'):
            if params.networkType.startswith('1dcnn'):
                print('using 1DCNN for raw data and FCN for freqHisto with time')
                model = self.generate1DCNNForRawDataWithFCNForFreqHistoWithTime(params)
            elif params.networkType.startswith('simple_cnn'):
                print('using simple CNN for raw data and FCN for freqHisto with time')
                model = self.generateSimpleCNNWithFCNForFreqHistoWithTime(params)
            elif params.networkType.startswith('cnn_lstm'):
                print('using CNN-LSTM with CNN for raw data and FCN for freqHisto with time')
    ...
    def fit(self, model, optimizer, train_loader, val_loader, epochs, steps_per_epoch, batch_size):
        print('starting fit()')
        # self.criterion = nn.CrossEntropyLoss()
        if torch.cuda.is_available():
    ...
    def train(self, featuresBySamples, labels):
          ...
          if validateRatio > 0:
              ...
              trainIDends = np.int(np.floor(sampleNum * (1-validateRatio)))
